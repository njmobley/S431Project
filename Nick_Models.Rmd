...
```{r}
lm.better2 <- lm(SRS_TotalRawScore ~ mABC_Balance.Component.StandardScore, data = kki.adhd2)
summary(lm.better2)
```

```{r}
ggplot(kki.adhd2,aes(x= mABC_TotalStandardScore, y = SRS_TotalRawScore)) + geom_point() + geom_jitter() + geom_smooth(colour = 'red') + geom_smooth(method = 'lm')
```


```{r}
ggplot(kki.adhd1,aes(x= mABC_Balance.Component.StandardScore, y = SRS_TotalRawScore)) + geom_point() + geom_jitter() + geom_smooth(method = 'lm') +geom_smooth(colour = 'red')
```

```{r}
ggplot(data = kki.adhd,aes(x = mABC_AGE,y = SRS_TotalRawScore)) + geom_point() + geom_smooth(method = 'lm')
```
